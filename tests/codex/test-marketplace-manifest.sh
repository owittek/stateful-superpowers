...
assert_equal(marketplace.get("name"), "stateful-superpowers-dev", "marketplace name")
assert_equal(
    marketplace.get("interface", {}).get("displayName"),
    "Stateful Superpowers Dev",
...
matching_plugins = [plugin for plugin in plugins if plugin.get("name") == "stateful-superpowers"]
